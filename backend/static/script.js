  ...
  Promise.all([
    ...
    .then(([playersData, playerStats]) => {
      ...
      const statsMap = new Map(
        playerStats.map((stat) => [stat.ID.toLowerCase(), stat])
      );

      // Determine team based on URL
      const teamName = window.location.pathname
        .toLowerCase()
        .includes("barcelona")
        ? "Barcelona"
        : "Real Madrid";
      ...
      positions.forEach((position) => {
        ...
        playersInPosition.forEach((player) => {
          ...
          if (player.position === "Goalkeeper") {
            playerStatsHTML += `<p class="stat clean-sheets"><strong>Clean Sheets:</strong> ${
              stat["Clean Sheets"] ?? "/"
            }</p>`;
            playerStatsHTML += `<p class="stat penalties-saved"><strong>Penalties Saved:</strong> ${
      ...
      if (searchInput) {
        searchInput.addEventListener("input", () => {
          ...
          const allPlayerCards =
            playerContainer.querySelectorAll(".player-card");
